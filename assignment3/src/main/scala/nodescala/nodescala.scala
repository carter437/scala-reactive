...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
      async{
        @tailrec
        def chunk() : Unit = {
          println(s"Response has next ${response.hasNext} && ${token.nonCancelled}")
          if(response.hasNext && token.nonCancelled){
            exchange.write(response.next())
            chunk()
          }
          else {
            println("Step 6: Closing Exchange")
            exchange.close()
          }
        }
        chunk()
...
/*        val l = createListener(relativePath)
        ...
        Future.run(){token =>
          ...
          l.nextRequest().map{ case (request,exchange) =>
             println("Step 5: Future Complete")
             respond(exchange,token,handler(request))
        }
     }*/
  }
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      println("Step 1: Next Request")
      val p = Promise[(Request,Exchange)]()

      createContext{ handler =>
        println("Step 3: Completing Request")
        p.complete(Try(handler.request,handler))
      }

      val f = p.future

      f onComplete { _ =>
        println("Step 4: Removing Context")
        removeContext()
      }

      println("Step 2: Returning Future")
      f
    }
  }
