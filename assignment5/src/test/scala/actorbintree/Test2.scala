...
class NodeCopyToTestSuite(_system: ActorSystem) extends TestKit(_system) with FunSuite with ShouldMatchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  test("empty node with two children confirms copy while actually copying to new node") {
    val nodeUnderTest = system.actorOf(BinaryTreeNode.props(0, initiallyRemoved = true),"root")
    val newNode = system.actorOf(BinaryTreeNode.props(1, initiallyRemoved = true),"newRoot")

    println(s"Test Actor: $testActor")
    println(s"Node Under Test: $nodeUnderTest")
    println(s"New Node Under Test: $newNode")
