...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuite with ShouldMatchers with BeforeAndAfterAll with ImplicitSender 
{
  ...
  def receiveN(requester: TestProbe, ops: Seq[Operation], expectedReplies: Seq[OperationReply]): Unit =
    requester.within(5.seconds) {
      val repliesUnsorted = for (i <- 1 to ops.size) yield try {
       // println(ops(i),expectedReplies(i),i)
  ...
  ignore("Leaf Nodes") {
    ...
    val ops = List(
      Insert(requesterRef, id=0, 42),
      Insert(requesterRef, id=1, -15),
      Insert(requesterRef, id=2, 2),
      Insert(requesterRef, id=3, 1),
      Contains(requesterRef, id=4, 1),
      Remove(requesterRef, id=5, 1),
  ...
  ignore("Remove Non-Existing Nodes") {
    val requester = TestProbe()
    val requesterRef = requester.ref
    val ops = List(
      Remove(requesterRef, id=1, 1)
    )
  ...
  ignore("Duplicate Inserts") {
    ...
    val ops = List(
      Insert(requesterRef, id=0, 42),
      Insert(requesterRef, id=1, 3),
      Insert(requesterRef, id=2, 42)
    )
  ...
  test("GC") {
    ...
    topNode ! Insert(testActor, id = 3, 4)
    expectMsg(OperationFinished(3))

    topNode ! Insert(testActor, id = 4, 8)
    expectMsg(OperationFinished(4))

    topNode ! Remove(testActor, id = 5, 4)
    expectMsg(OperationFinished(5))

    topNode ! Remove(testActor, id = 20, 4)
    ...
    topNode ! Contains(testActor, id = 8, 4)
    expectMsg(ContainsResult(8, false))

    topNode ! Contains(testActor, id = 9, 8)
  ...
  test("behave identically to built-in set (includes GC)") {
    ...
    ops foreach { op =>
      topNode ! op
      if (rnd.nextDouble() < 0.5) topNode ! GC
    }
