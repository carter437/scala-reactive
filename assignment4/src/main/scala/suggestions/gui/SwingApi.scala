...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = {
      Observable.apply{ observer:Observer[String] =>
        val rx: Reaction = { case ValueChanged(tf) => observer.onNext(tf.text) }
        field.subscribe(rx)
        Subscription{
          field.unsubscribe(rx)
        }
      }
    }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = {
      Observable.apply{ observer:Observer[Button] =>
         val rx : Reaction = { case ButtonClicked(btn) => observer.onNext(btn) }
         button.subscribe(rx)
         Subscription{
           button.unsubscribe(rx)
         }
      }
    }
