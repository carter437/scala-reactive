...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val searchTerms: Observable[String] = Observable.apply{ observer:Observer[String] =>
      val rx: Reaction = { case ValueChanged(tf) => observer.onNext(tf.text) }
      searchTermField.subscribe(rx)
      Subscription{
        searchTermField.unsubscribe(rx)
      }
    }

    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = {
      Observable.apply{ observer:Observer[Try[List[String]]] =>
         val pub =  AsyncSubject[Try[List[String]]]
         searchTerms.subscribe(
          onNext = str => wikipediaSuggestion(str) onComplete{s => pub.onNext(s);pub.onCompleted()},
          onError = t => pub.onError(t),
          onCompleted = () => pub.onCompleted()
         )
         Subscription{}
      }
    }
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = Observable.apply(new UnsupportedOperationException())

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = Observable.apply(new UnsupportedOperationException())
