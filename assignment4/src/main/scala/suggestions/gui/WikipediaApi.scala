...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      obs.materialize.filter{
          case OnCompleted(x) => false
          case _ => true
      }.map{
        case OnNext(v) =>
          v match {
...
/*     //obs.map(Success(_)).onErrorReturn(Failure(_))
      Observable.apply{o: Observer[Try[T]] =>
         obs.materialize.filter{
           case OnCompleted(x) => { println(s"------------ $x");false}
           case _ => true
         }.subscribe(n => n match {
           case OnNext(v) => o.onNext(Success(v))
           case OnError(t) => { o.onNext(Failure(t));o.onCompleted()}
         })
         Subscription{}
      }
    }*/
     ...
     /* obs.map{ v =>
        val ret: Try[T] = Success(v);
        ret
      }.onErrorResumeNext(e => Observable(Failure(e)))
    } */
